...
void XMLDocumentParser::append(const String& inputSource)
{
    ...
    if (document()->isWMLDocument() && !m_parsedFirstWMLFragment) {
        m_parsedFirstWMLFragment = true;
        document()->getSecurityOrigin()->grantUniversalAccess();
        document()->getSecurityOrigin()->grantAccessInsecureContent();
        DEFINE_STATIC_LOCAL(String, xmlDecl, ("<?xml"));
        DEFINE_STATIC_LOCAL(String, wmlTag, ("<wml"));
        DEFINE_STATIC_LOCAL(String, xslInsertion, ("<?xml-stylesheet type=\"text/xsl\" href=\"chrome://resources/wml/swe_wml.xsl\"?>\n"));
        size_t xmlDeclStart = data.findIgnoringCase(xmlDecl);
        // Remove all the junk before the xml declaration.
        if (xmlDeclStart > 0)
            data.remove(0, xmlDeclStart);
        // Insert WML XSL sheet before <wml> tag.
        size_t wmlStart = data.findIgnoringCase(wmlTag);
        if (wmlStart > 0)
            data.insert(xslInsertion, wmlStart);
    }
    SegmentedString source(data);
...
static bool shouldAllowExternalLoad(const KURL& url)
{
    ...
    if (XMLDocumentParserScope::currentDocument->isWMLDocument()) {
        if (urlString.startsWith("http://www.wapforum.org/DTD", TextCaseInsensitive)
            || urlString.startsWith("http://www.openwave.com/DTD", TextCaseInsensitive)
            || urlString.startsWith("http://www.phone.com/DTD", TextCaseInsensitive))
            return false;
    }
...
XMLDocumentParser::XMLDocumentParser(Document& document, FrameView* frameView)
    ...
    , m_parsedFirstWMLFragment(false)
{
