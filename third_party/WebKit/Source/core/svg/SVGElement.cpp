...
{
    // Number of SVG elements that indicates heavy workload
    const int HEAVY_SVG_THRESHOLD = 4000;

    // Expected heavy workload time, in milliseconds
    const int HEAVY_SVG_DURATION = 14000;

    if (document.getSvgCounter() < HEAVY_SVG_THRESHOLD) {
        document.incrementSvgCounter();
        if (HEAVY_SVG_THRESHOLD == document.getSvgCounter()) {
            document.heavyExecutionDetected(HEAVY_SVG_DURATION, true);
        }
    }
