...
void LayoutMenuList::setTextFromOption(int optionIndex)
{
    ...
    if (selectElement()->multiple()) {
        ...
        //decide which option items in select element should be shown to user
        int sizeShown = std::min((int)select->size(), size);
        if (sizeShown < 2) {
        ...
        if (i <= 0) {
            text = "";
            for (int j = 0; j< sizeShown; j++) {
                Element* element = listItems[j];
                if (!isHTMLOptionElement(*element))
                    continue;
                if (HTMLOptionElement* optionElement = toHTMLOptionElement(element)) {
                    text = text + "\n" + optionElement->textIndentedToRespectGroupLabel();
                }
            }
        } else if (i == size - 1) {
            text = "";
            for (int j = size - 1; j > size - sizeShown - 1; j--) {
                Element* element = listItems[j];
                if (!isHTMLOptionElement(*element))
                    continue;
                if (HTMLOptionElement* optionElement = toHTMLOptionElement(element)) {
