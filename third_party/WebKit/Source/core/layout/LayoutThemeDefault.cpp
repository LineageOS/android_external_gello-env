...
void LayoutThemeDefault::adjustListboxStyle(ComputedStyle& style, Element* e) const
{
    const int listboxPadding = 5;
    HTMLSelectElement* select = toHTMLSelectElement(e);
    int size = select->listItems().size();
    int sizeShown = std::min(size, (int)select->size());
    // adjust height for select element to hold not only one option
    if (select && sizeShown > 1) {
        int oldHeight = style.logicalHeight().isFixed() ? style.logicalHeight().value() : -1;
        adjustMenuListButtonStyle(style, 0);
        style.setHeight(Length(oldHeight > 0 ? oldHeight :
                (style.fontSize() + listboxPadding) * sizeShown, Fixed));
    } else {
        adjustMenuListButtonStyle(style, 0);
    }
}
