...
enum StyleChangeState {
    NoStyleChangeState = 0x00,
    ChildrenAffectedByBackForwardPositionalRules = 0x01,
    AffectedByDirectAdjacentAndPositionalRules = 0x02,
    NewNodeStyleChangeState = 0x04,
    HasScopedRecalcStyle = 0x08,
};
...
public:
    ...
    static const char *COMPAT_LIB_NAMES[];
    ...
    static bool inline isArchitectureCompatible() {

#if OS(ANDROID)
        static int compat = -1;
        if (-1 == compat) {
            compat = 0;
            for (int i = 0; COMPAT_LIB_NAMES[i]; i++) {
                void *handle = ::dlopen(COMPAT_LIB_NAMES[i], RTLD_LAZY);
                if (handle) {
                    compat = 1;
                    ::dlclose(handle);
                    break;
    ...
    void setStyleChangeState(StyleChangeState);
    void clearStyleChangeState();
    uint32_t styleChangeState() const { return m_styleChangeState; }
    ...
    void setNeedsStyleRecalc(StyleChangeType, const StyleChangeReasonForTracing&, StyleChangeState = NoStyleChangeState);
