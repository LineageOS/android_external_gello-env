...
void Element::recalcStyle(StyleRecalcChange change, Text* nextTextSibling, bool scoped)
{
    ...
    DCHECK(scoped || !parentOrShadowHostNode()->needsStyleRecalc());
...
StyleRecalcChange Element::recalcOwnStyle(StyleRecalcChange change, bool scoped)
{
    ...
    RefPtr<ComputedStyle> newStyle = !scoped ? styleForLayoutObject() :
        document().ensureStyleResolver().styleForElement(this, 0, DisallowStyleSharing);
    DCHECK(newStyle);
