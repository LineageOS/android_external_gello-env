...
const char *Node::COMPAT_LIB_NAMES[] = {
...
Node::Node(TreeScope* treeScope, ConstructionType type)
    : m_nodeFlags(type)
    , m_styleChangeState(NewNodeStyleChangeState)
...
void Node::setStyleChangeState(StyleChangeState state)
{
    m_styleChangeState |= state;
}

void Node::clearStyleChangeState()
{
    m_styleChangeState = NoStyleChangeState;
}
...
void Node::setNeedsStyleRecalc(StyleChangeType changeType, const StyleChangeReasonForTracing& reason, StyleChangeState state)
{
