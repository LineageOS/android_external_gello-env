    ...
    , m_nodeCount(0)
{
    ...
    WebActivities::get()->createActivityData(&m_activityData, WebActivityDataType::DOMDocumentType);
...
void Document::heavyExecutionDetected(int duration, bool pageLoad) const
{
    Page *page = this->page();
    if (page)
        page->heavyExecutionDetected(duration, pageLoad);
}
