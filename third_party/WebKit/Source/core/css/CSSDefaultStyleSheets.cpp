...
CSSDefaultStyleSheets::CSSDefaultStyleSheets()
    ...
    , m_defaultExtraStyle01(nullptr)
    , m_defaultExtraStyle02(nullptr)
    ...
    , m_defaultExtraStyleSheet01(nullptr)
    , m_defaultExtraStyleSheet02(nullptr)
...
void CSSDefaultStyleSheets::addDefaultExtraStyleSheet01(const String& styleRules)
{
    if (!m_defaultExtraStyle01) {
        m_defaultExtraStyle01 = RuleSet::create();
        m_defaultExtraStyleSheet01 = parseUASheet(styleRules);
        m_defaultExtraStyle01->addRulesFromSheet(defaultExtraStyleSheet01(), screenEval());
    }
}

void CSSDefaultStyleSheets::addDefaultExtraStyleSheet02(const String& styleRules)
{
    if (!m_defaultExtraStyle02) {
        m_defaultExtraStyle02 = RuleSet::create();
        m_defaultExtraStyleSheet02 = parseUASheet(styleRules);
        m_defaultExtraStyle02->addRulesFromSheet(defaultExtraStyleSheet02(), screenEval());
    }
}
...
DEFINE_TRACE(CSSDefaultStyleSheets)
{
    ...
    visitor->trace(m_defaultExtraStyle01);
    visitor->trace(m_defaultExtraStyle02);
    ...
    visitor->trace(m_defaultExtraStyleSheet01);
    visitor->trace(m_defaultExtraStyleSheet02);
