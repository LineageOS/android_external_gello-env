...
{
    if (pseudoElementName.isNull()) {
        m_pseudoElementSpecifier = PseudoIdNone;
    } else {
...
CSSValue* CSSComputedStyleDeclaration::getPropertyCSSValue(CSSPropertyID propertyID) const
{
    ...
    bool recalcStyleGlobal = true;
    Node& rootNode = rootInComposedTree(*styledNode);
    bool needsDistrubutionRecalc = rootNode.childNeedsDistributionRecalc();
    rootNode.clearChildNeedsDistributionRecalc();
    Node* ancestor = styledNode->parentOrShadowHostNode();
    if (ancestor
        && !isTreeScopeRoot(ancestor)
        && !styledNode->isInShadowTree()
        && !styledNode->parentElementShadow()
        && !styledNode->containingShadowRoot()
        && styledNode->isElementNode()
        && styledNode->parentComputedStyle()) {
        bool canDoStrictRecalc = false;
        if (ancestor->needsStyleRecalc()
            && (ancestor->styleChangeState() == ChildrenAffectedByBackForwardPositionalRules))
            canDoStrictRecalc = true;
        else if (!ancestor->needsStyleRecalc()) {
            bool invalidateAncestorStyle = false;
            for (Node* a = ancestor; a && !invalidateAncestorStyle; a = a->parentOrShadowHostNode())
                invalidateAncestorStyle = a->needsStyleRecalc() || a->childNeedsDistributionRecalc();
            if (!invalidateAncestorStyle)
                canDoStrictRecalc = true;
        }
        if (canDoStrictRecalc) {
            Element* styledElement = toElement(styledNode);
            if (styledNode->needsStyleRecalc()
                && styledNode->styleChangeState() == NewNodeStyleChangeState
                && !styledElement->hasCustomStyleCallbacks()
                && !styledElement->elementAnimations()) {
                DocumentLifecycle::LifecycleState oldState = document.lifecycle().state();
                document.lifecycle().advanceTo(DocumentLifecycle::InStyleRecalc);
                {
                    SelectorFilterParentScope filterScope(*document.documentElement());
                    styledElement->recalcStyle(NoChange, 0, true);
                }
                document.lifecycle().rewindTo(oldState);
                recalcStyleGlobal = false;
            }
            else if (!styledNode->needsStyleRecalc()
                && (styledNode->styleChangeState() & HasScopedRecalcStyle))
                recalcStyleGlobal = false;
        }
    }
    if (needsDistrubutionRecalc)
        rootNode.setChildNeedsDistributionRecalc();
    if (recalcStyleGlobal)
...
{
    DEFINE_STATIC_LOCAL(String, unknownPropertyName, (""));
    if (unknownPropertyName.impl() == propertyName.impl())
        return String();
    ...
    if (!propertyID) {
        ...
        unknownPropertyName = propertyName;
        return String();
    }
