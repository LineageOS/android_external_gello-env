...
void StartTilingQCOM(GLuint x,
  ...
  gles2::cmds::StartTilingQCOM* c = GetCmdSpace<gles2::cmds::StartTilingQCOM>();
  if (c) {
    c->Init(x, y, width, height, preserveMask);
  }
}

void EndTilingQCOM(GLbitfield preserveMask) {
  gles2::cmds::EndTilingQCOM* c = GetCmdSpace<gles2::cmds::EndTilingQCOM>();
  if (c) {
    c->Init(preserveMask);
  }
}
