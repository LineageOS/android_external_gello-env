...
TEST_F(GLES2ImplementationTest, StartTilingQCOM) {
  struct Cmds {
    cmds::StartTilingQCOM cmd;
  };
  Cmds expected;
  expected.cmd.Init(1, 2, 3, 4, 5);

  gl_->StartTilingQCOM(1, 2, 3, 4, 5);
  EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}

TEST_F(GLES2ImplementationTest, EndTilingQCOM) {
  struct Cmds {
    cmds::EndTilingQCOM cmd;
  };
  Cmds expected;
  expected.cmd.Init(1);
