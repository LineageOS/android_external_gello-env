...
void GLES2Implementation::StartTilingQCOM(GLuint x,
                                          GLuint y,
                                          GLuint width,
                                          GLuint height,
                                          GLbitfield preserveMask) {
  GPU_CLIENT_SINGLE_THREAD_CHECK();
  GPU_CLIENT_LOG("[" << GetLogPrefix() << "] glStartTilingQCOM(" << x << ", "
                     << y << ", " << width << ", " << height << ", "
                     << preserveMask << ")");
  helper_->StartTilingQCOM(x, y, width, height, preserveMask);
  CheckGLError();
}

void GLES2Implementation::EndTilingQCOM(GLbitfield preserveMask) {
  GPU_CLIENT_SINGLE_THREAD_CHECK();
  GPU_CLIENT_LOG("[" << GetLogPrefix() << "] glEndTilingQCOM(" << preserveMask
                     << ")");
  helper_->EndTilingQCOM(preserveMask);
  CheckGLError();
}
