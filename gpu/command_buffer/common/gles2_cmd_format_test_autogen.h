...
TEST_F(GLES2FormatTest, StartTilingQCOM) {
  cmds::StartTilingQCOM& cmd = *GetBufferAs<cmds::StartTilingQCOM>();
  void* next_cmd =
      cmd.Set(&cmd, static_cast<GLuint>(11), static_cast<GLuint>(12),
              static_cast<GLuint>(13), static_cast<GLuint>(14),
              static_cast<GLbitfield>(15));
  EXPECT_EQ(static_cast<uint32_t>(cmds::StartTilingQCOM::kCmdId),
            cmd.header.command);
  EXPECT_EQ(sizeof(cmd), cmd.header.size * 4u);
  EXPECT_EQ(static_cast<GLuint>(11), cmd.x);
  EXPECT_EQ(static_cast<GLuint>(12), cmd.y);
  EXPECT_EQ(static_cast<GLuint>(13), cmd.width);
  EXPECT_EQ(static_cast<GLuint>(14), cmd.height);
  EXPECT_EQ(static_cast<GLbitfield>(15), cmd.preserveMask);
  CheckBytesWrittenMatchesExpectedSize(next_cmd, sizeof(cmd));
}

TEST_F(GLES2FormatTest, EndTilingQCOM) {
  cmds::EndTilingQCOM& cmd = *GetBufferAs<cmds::EndTilingQCOM>();
  void* next_cmd = cmd.Set(&cmd, static_cast<GLbitfield>(11));
  EXPECT_EQ(static_cast<uint32_t>(cmds::EndTilingQCOM::kCmdId),
            cmd.header.command);
  EXPECT_EQ(sizeof(cmd), cmd.header.size * 4u);
  EXPECT_EQ(static_cast<GLbitfield>(11), cmd.preserveMask);
  CheckBytesWrittenMatchesExpectedSize(next_cmd, sizeof(cmd));
}
