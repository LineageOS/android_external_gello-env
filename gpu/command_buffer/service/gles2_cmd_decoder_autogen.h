...
error::Error GLES2DecoderImpl::HandleStartTilingQCOM(
    uint32_t immediate_data_size,
    const void* cmd_data) {
  const gles2::cmds::StartTilingQCOM& c =
      *static_cast<const gles2::cmds::StartTilingQCOM*>(cmd_data);
  (void)c;
  GLuint x = static_cast<GLuint>(c.x);
  GLuint y = static_cast<GLuint>(c.y);
  GLuint width = static_cast<GLuint>(c.width);
  GLuint height = static_cast<GLuint>(c.height);
  GLbitfield preserveMask = static_cast<GLbitfield>(c.preserveMask);
  DoStartTiling(x, y, width, height, preserveMask);
  return error::kNoError;
}

error::Error GLES2DecoderImpl::HandleEndTilingQCOM(uint32_t immediate_data_size,
                                                   const void* cmd_data) {
  const gles2::cmds::EndTilingQCOM& c =
      *static_cast<const gles2::cmds::EndTilingQCOM*>(cmd_data);
  (void)c;
  GLbitfield preserveMask = static_cast<GLbitfield>(c.preserveMask);
  DoEndTiling(preserveMask);
  return error::kNoError;
}
