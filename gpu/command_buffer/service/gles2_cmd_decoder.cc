...
class GLES2DecoderImpl : public GLES2Decoder, public ErrorStateClient {
 ...
 private:
  ...
  void DoStartTiling(GLuint x, GLuint y, GLuint width, GLuint height, GLbitfield preserveMask);
  void DoEndTiling (GLbitfield preserveMask);
...
Capabilities GLES2DecoderImpl::GetCapabilities() {
  ...
  caps.use_tiled_rendering = feature_info_->feature_flags().use_tiled_rendering;
#endif
...
void GLES2DecoderImpl::DoStartTiling(GLuint x, GLuint y, GLuint width, GLuint height, GLbitfield preserveMask) {
#ifdef DO_TILED_RENDERING
  if (!features().use_tiled_rendering) {
    LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, "glStartTilingQCOM",
                       "function not available");
    return;
  }
  if (!surface_->CanDoTiling()) {
    ZEROCOPY_LOG_PARTIAL_COMPOSITION("GLES2DecoderImpl::DoStartTiling not tiling");
    return;
  }
  state_.doing_tiling = true;
  state_.start_tiling_x = x;
  state_.start_tiling_y = y;
  state_.start_tiling_w = width;
  state_.start_tiling_h = height;
  state_.start_tiling_preserveMask = preserveMask;
  ZEROCOPY_LOG_PARTIAL_COMPOSITION("GLES2DecoderImpl::DoStartTiling tiling %d, %d, %d %d", x, y, width, height);
  glStartTilingQCOM(x, y, width, height, preserveMask);
#endif
}

void GLES2DecoderImpl::DoEndTiling (GLbitfield preserveMask) {
#ifdef DO_TILED_RENDERING
  if (!features().use_tiled_rendering) {
    LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, "glStartTilingQCOM",
                       "function not available");
  ...
  if (state_.doing_tiling) {
    state_.doing_tiling = false;
