...
bool GLContextVirtual::Initialize(
  ...
  if (!IsCurrent(compatible_surface)) {
    ...
    if (current_surface != compatible_surface) {
      //if not changing the underlying surface, but the GLSurface is changed,
      //notify the old surface as whatever it was rendering may be overwritten
      ZEROCOPY_LOG_PARTIAL_COMPOSITION("VirtualGLApi::MakeCurrent OnMakeNotCurrent");
      current_surface->OnMakeNotCurrent();
    }
  }
#endif
