...
void WebMediaPlayerImpl::adjustBrightness(float delta) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());

  if (delegate_)
      delegate_->AdjustBrightness(delta);
}

void WebMediaPlayerImpl::setRotateLock(bool lock) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());

  if (delegate_)
      delegate_->SetRotateLock(lock);
}
...
void WebMediaPlayerImpl::OnDeltaSeek(int delta) {
  seek(currentTime() + delta);
}

void WebMediaPlayerImpl::OnBrightnessChanged(float brightness) {
...
WebMediaPlayerImpl::UpdatePlayState_ComputePlayState(bool is_remote,
  ...
  bool background_suspended = is_backgrounded && have_metadata &&
      (fullscreen_ || !hasAudio() ||
          (delegate_ && !delegate_->ShouldAllowBackgroundAudio()));
