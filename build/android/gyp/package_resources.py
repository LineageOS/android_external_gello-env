...
def PackageArgsForExtractedZip(d):
  ...
  for d in res_dirs:
    ...
    subsubdirs = [os.path.join(d, s) for s in os.listdir(d)]
    subsubdirs = [s for s in subsubdirs if os.path.isdir(s)]
    is_multi = '0' in [os.path.basename(s) for s in subsubdirs]
    if is_multi:
      subsubdirs = sorted(subsubdirs, key=lambda p : int(os.path.basename(p)))
      for sub_d in subsubdirs:
        package_command += ['-S', sub_d]
    else:
      package_command += ['-S', d]
