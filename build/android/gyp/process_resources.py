...
def _ParseArgs(args):
  ...
  parser.add_option(
      '--resource-extra-dirs',
      help='Overlay resources directories')
  ...
  if options.resource_extra_dirs:
    options.resource_extra_dirs = build_utils.ParseGypList(options.resource_extra_dirs)
  else:
    options.resource_extra_dirs = []
...
def CombineZipResources(resource_dirs, zip_path):
  # Create zipped file for each resource directories
  zipped_dirs = []
  for i, d in enumerate(resource_dirs):
    zip_out_path = os.path.splitext(zip_path)[0] + str(i) + "_i.zip"
    ZipResources([d], zip_out_path)
    zipped_dirs.append(zip_out_path)
  return zipped_dirs
...
def _OnStaleMd5(options):
  ...
  with build_utils.TempDir() as temp_dir:
    ...
    input_resource_extra_dirs = options.resource_extra_dirs
    ...
    for z in dep_zips:
      ...
      for root, directories, files in os.walk(subdir):
        ...
        for dir in directories:
          try:
            if int(dir) in r:
               dep_subdirs.append(subdir+"/"+str(dir))
               has_multiple_res_dir = True
          except ValueError:
            break
      if not has_multiple_res_dir:
    ...
    if options.resource_extra_dirs:
      for d in input_resource_extra_dirs:
        package_command += ['-S', d]
    ...
    tmp_zip = options.resource_zip_out + "_xyz.zip"
    ZipResources(zip_resource_dirs, options.resource_zip_out)

    if options.resource_extra_dirs:
      ZipResources(zip_resource_dirs, tmp_zip)
      zip_out_paths = [tmp_zip]
      extra_res_zip_out =  CombineZipResources(input_resource_extra_dirs,
          options.resource_zip_out)
      zip_out_paths = extra_res_zip_out + zip_out_paths
      CombineZips(extra_res_zip_out, options.resource_zip_out)
    else:
      ZipResources(zip_resource_dirs, options.resource_zip_out)
...
def main(args):
  ...
  for resource_dir in options.resource_extra_dirs:
    for resource_file in build_utils.FindInDirectory(resource_dir, '*'):
      input_paths.append(resource_file)
      resource_names.append(os.path.relpath(resource_file, resource_dir))
