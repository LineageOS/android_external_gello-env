...
void URLMatcherConditionFactory::clear() {
  STLDeleteElements(&substring_pattern_singletons_);
  STLDeleteElements(&regex_pattern_singletons_);
  STLDeleteElements(&origin_and_path_regex_pattern_singletons_);
}
...
URLMatcher::URLMatcher(bool have_single_condition)
    : url_matcher_condition_sets_have_single_condition_(have_single_condition)
    , full_url_matcher_(have_single_condition)
    , url_component_matcher_(have_single_condition)
    , regex_set_matcher_(have_single_condition)
    , origin_and_path_regex_set_matcher_(have_single_condition) {}
...
void URLMatcher::AddConditionSets(
  ...
  if (url_matcher_condition_sets_have_single_condition_)
      condition_factory()->clear();
}
...
void URLMatcher::UpdateInternalDatastructures() {
  ...
  UpdateConditionFactory();
  if (url_matcher_condition_sets_have_single_condition_)
      url_matcher_condition_sets_.clear();
}
