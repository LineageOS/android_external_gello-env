...
MockGLInterface::Mock_glEndTilingQCOM(GLbitfield preserveMask) {
  MakeFunctionUnique("glEndTilingQCOM");
...
MockGLInterface::Mock_glStartTilingQCOM(GLuint x,
  ...
  MakeFunctionUnique("glStartTilingQCOM");
...
void* GL_BINDING_CALL MockGLInterface::GetGLProcAddress(const char* name) {
  ...
  if (strcmp(name, "glEndTilingQCOM") == 0)
    return reinterpret_cast<void*>(Mock_glEndTilingQCOM);
  ...
  if (strcmp(name, "glStartTilingQCOM") == 0)
    return reinterpret_cast<void*>(Mock_glStartTilingQCOM);
