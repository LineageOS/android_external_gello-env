...
bool g_gl_tiled_rendering_supported = false;
bool g_gl_tiled_rendering_queried = false;
#endif
...
#ifdef DO_TILED_RENDERING
  EGLint preserved = 0;
  if (gfx::g_driver_gl.ext.b_GL_QCOM_tiled_rendering) {
    preserved = EGL_SWAP_BEHAVIOR_PRESERVED_BIT;
  }
#endif
...
#ifdef DO_TILED_RENDERING
void GLSurfaceEGL::OnMakeNotCurrent() {
  can_do_tiling_ = false;
}

bool GLSurfaceEGL::CanDoTiling() {
  return can_do_tiling_;
}
#endif
...
bool NativeViewGLSurfaceEGL::Initialize(
  ...
  if (gfx::g_driver_egl.ext.b_EGL_KHR_swap_buffers_with_damage) {
    ZEROCOPY_LOG_PARTIAL_SWAP("EGL_KHR_swap_buffers_with_damage supported");
    supports_post_sub_buffer_ = EGL_TRUE;
  } else {
    ZEROCOPY_LOG_PARTIAL_SWAP("EGL_KHR_swap_buffers_with_damage NOT supported");
  }
#endif
...
#ifdef DO_TILED_RENDERING
  if (!g_gl_tiled_rendering_queried) {
    g_gl_tiled_rendering_queried = true;
    GLContext* context = GLContext::GetCurrent();
    if (!context) {
      ZEROCOPY_LOG_PARTIAL_COMPOSITION("NativeViewGLSurfaceEGL::Initialize no current context");
      scoped_refptr<GLContext> new_context = GLContext::CreateGLContext(NULL, this, PreferIntegratedGpu);
      if (!new_context.get()) {
        ZEROCOPY_LOG_PARTIAL_COMPOSITION("    can't create context");
      } else {
        if (!new_context->MakeCurrent(this)) {
          ZEROCOPY_LOG_PARTIAL_COMPOSITION("      can't make current");
        } else {
          g_gl_tiled_rendering_supported = new_context->HasExtension("GL_QCOM_tiled_rendering");
          new_context->ReleaseCurrent(this);
        }
      }
    } else {
      g_gl_tiled_rendering_supported = context->HasExtension("GL_QCOM_tiled_rendering");
    }
  }
  if (g_gl_tiled_rendering_supported) {
    ZEROCOPY_LOG_PARTIAL_COMPOSITION("NativeViewGLSurfaceEGL::Initialize preserved");
    eglSurfaceAttrib(GetDisplay(), surface_, EGL_SWAP_BEHAVIOR, EGL_BUFFER_PRESERVED);
  } else {
    ZEROCOPY_LOG_PARTIAL_COMPOSITION("NativeViewGLSurfaceEGL::Initialize not preserved");
  }
...
#ifdef DO_TILED_RENDERING
  ZEROCOPY_LOG_PARTIAL_COMPOSITION("NativeViewGLSurfaceEGL::SwapBuffers setting tiling");
  can_do_tiling_ = true;
#endif
...
#ifdef DO_PARTIAL_SWAP
  ZEROCOPY_LOG_PARTIAL_SWAP("NativeViewGLSurfaceEGL::PostSubBuffer (%d, %d, %d, %d)", x, y, width, height);
  if (gfx::g_driver_egl.ext.b_EGL_KHR_swap_buffers_with_damage) {
    EGLint rect[4];
    rect[0] = x;
    rect[1] = y;
    rect[2] = width;
    rect[3] = height;
    eglSwapBuffersWithDamageKHR(GetDisplay(), surface_, rect, 1);
  } else {
...
#ifdef DO_TILED_RENDERING
  ZEROCOPY_LOG_PARTIAL_COMPOSITION("NativeViewGLSurfaceEGL::PostSubBuffer setting tiling");
  can_do_tiling_ = true;
#endif
