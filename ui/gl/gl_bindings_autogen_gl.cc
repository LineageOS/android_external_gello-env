...
void DriverGL::InitializeDynamicBindings(GLContext* context) {
  ...
  ext.b_GL_QCOM_tiled_rendering =
      extensions.find("GL_QCOM_tiled_rendering ") != std::string::npos;
  ...
  debug_fn.glStartTilingQCOMFn = 0;
  if (ext.b_GL_QCOM_tiled_rendering) {
...
static void GL_BINDING_CALL Debug_glEndTilingQCOM(GLbitfield preserveMask) {
  GL_SERVICE_LOG("glEndTilingQCOM"
                 << "(" << preserveMask << ")");
  g_driver_gl.debug_fn.glEndTilingQCOMFn(preserveMask);
}
...
static void GL_BINDING_CALL Debug_glStartTilingQCOM(GLuint x,
                                                    GLuint y,
                                                    GLuint width,
                                                    GLuint height,
  ...
  GL_SERVICE_LOG("glStartTilingQCOM"
                 << "(" << x << ", " << y << ", " << width << ", " << height
                 << ", " << preserveMask << ")");
  g_driver_gl.debug_fn.glStartTilingQCOMFn(x, y, width, height, preserveMask);
}
...
void DriverGL::InitializeDebugBindings() {
  ...
  if (!debug_fn.glEndTilingQCOMFn) {
    debug_fn.glEndTilingQCOMFn = fn.glEndTilingQCOMFn;
    fn.glEndTilingQCOMFn = Debug_glEndTilingQCOM;
  }
  ...
  if (!debug_fn.glStartTilingQCOMFn) {
    debug_fn.glStartTilingQCOMFn = fn.glStartTilingQCOMFn;
    fn.glStartTilingQCOMFn = Debug_glStartTilingQCOM;
  }
...
void NoContextGLApi::glEndTilingQCOMFn(GLbitfield preserveMask) {
  NOTREACHED() << "Trying to call glEndTilingQCOM() without current GL context";
  LOG(ERROR) << "Trying to call glEndTilingQCOM() without current GL context";
}
...
void NoContextGLApi::glStartTilingQCOMFn(GLuint x,
                                         ...
                                         GLbitfield preserveMask) {
  NOTREACHED()
      << "Trying to call glStartTilingQCOM() without current GL context";
  LOG(ERROR) << "Trying to call glStartTilingQCOM() without current GL context";
}
