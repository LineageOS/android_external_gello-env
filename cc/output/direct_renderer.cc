...
void DirectRenderer::DrawFrame(RenderPassList* render_passes_in_draw_order,
  ...
  if (!skip_drawing_root_render_pass && (!Capabilities().using_partial_swap
      ...
      && !Capabilities().using_swap_buffer_with_damage
#endif
    ))
...
void DirectRenderer::DrawRenderPass(DrawingFrame* frame,
  ...
  gfx::Rect tiling_rect = frame->root_render_pass->damage_rect;
  gfx::Size surface_size = output_surface_->SurfaceSize();
  tiling_rect.Intersect(gfx::Rect(surface_size));
  BeginDrawingRenderPass(frame, render_pass, MoveFromDrawToWindowSpace(frame, tiling_rect));
#endif
...
#ifdef DO_PARTIAL_COMPOSITION
  FinishDrawingRenderPass(frame, render_pass);
#endif
}
