...
GLRenderer::GLRenderer(RendererClient* client,
  ...
  capabilities_.using_swap_buffer_with_damage =
    context_caps.use_tiled_rendering && context_caps.post_sub_buffer;
#else
  capabilities_.using_swap_buffer_with_damage = context_caps.post_sub_buffer;
#endif
#endif
...
#ifdef DO_TILED_RENDERING
  capabilities_.use_tiled_rendering = context_caps.use_tiled_rendering;
#endif
...
#ifdef DO_TILED_RENDERING
  ZEROCOPY_LOG_PARTIAL_COMPOSITION("GLRenderer::BeginDrawingFrame damage rect %d, %d, %d, %d",
    frame->root_render_pass->damage_rect.x(), frame->root_render_pass->damage_rect.y(), frame->root_render_pass->damage_rect.right(), frame->root_render_pass->damage_rect.bottom());
#endif
...
void GLRenderer::BeginDrawingRenderPass(DrawingFrame* frame, const RenderPass* render_pass, const gfx::Rect& render_pass_scissor) {
#ifdef DO_TILED_RENDERING
  if (frame->root_render_pass == render_pass && capabilities_.use_tiled_rendering) {
    ZEROCOPY_LOG_PARTIAL_COMPOSITION("GLRenderer::BeginDrawingRenderPass StartTilingQCOM");
    gl_->StartTilingQCOM(render_pass_scissor.x(), render_pass_scissor.y(), render_pass_scissor.width(), render_pass_scissor.height(), GL_NONE);
  }
#endif
}
#endif
...
#ifdef DO_PARTIAL_COMPOSITION
void GLRenderer::FinishDrawingRenderPass(DrawingFrame* frame, const RenderPass* render_pass) {
#ifdef DO_TILED_RENDERING
  if (frame->root_render_pass == render_pass && capabilities_.use_tiled_rendering) {
    ZEROCOPY_LOG_PARTIAL_COMPOSITION("GLRenderer::BeginDrawingRenderPass EndTilingQCOM");
    gl_->EndTilingQCOM(GL_COLOR_BUFFER_BIT0_QCOM);
  }
#endif
}
#endif
