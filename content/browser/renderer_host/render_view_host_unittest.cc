...
class HeavyExecutionObserver : public WebContentsObserver {
  public:
    HeavyExecutionObserver(WebContents* web_contents)
      : WebContentsObserver(web_contents)
      , heavy_(false)
      { }
    void HeavyExecutionDetected(int duration) override { heavy_ = duration; };
    int Heavy() { return heavy_; }

  private:
    int heavy_;
};

TEST_F(RenderViewHostTest, HeavyExecution) {
  HeavyExecutionObserver observer(contents());
  test_rvh()->OnHeavyExecutionDetected(1234);

  EXPECT_EQ(1234, observer.Heavy());
}
