  ...
  void SwapBuffers(cc::CompositorFrame* frame) override {
    ...
    if (frame->gl_frame_data->sub_buffer_rect.IsEmpty()) {
...
#ifndef DO_PARTIAL_SWAP
      DCHECK(frame->gl_frame_data->sub_buffer_rect ==
          gfx::Rect(frame->gl_frame_data->size));
#else
      if (frame->gl_frame_data->sub_buffer_rect ==
          gfx::Rect(frame->gl_frame_data->size)) {
        ZEROCOPY_LOG_PARTIAL_SWAP("OutputSurfaceWithoutParent::SwapBuffers full swap");
#endif
      ...
      context_provider_->ContextSupport()->Swap();
        ...
        ZEROCOPY_LOG_PARTIAL_SWAP("OutputSurfaceWithoutParent::SwapBuffers partial swap");
