...
bool ChildProcessSecurityPolicyImpl::CanRequestURL(
  ...
  bool can_request = !GetContentClient()->browser()->IsHandledURL(url) &&
         !net::URLRequest::IsHandledURL(url);
  if (!can_request && url.SchemeIs(kChromeUIScheme)) {
    can_request = base::StartsWith(url.spec(), "chrome://resources/wml/swe_wml.", base::CompareCase::SENSITIVE);
  }
  return can_request;
}
