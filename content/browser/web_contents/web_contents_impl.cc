...
WebContentsImpl::WebContentsImpl(BrowserContext* browser_context)
  ...
  WebRefiner::Get()->RegisterWebContentsImpl(this);
}
...
void WebContentsImpl::DeltaSeekMediaSession(int delta) {
  MediaSession::Get(this)->DeltaSeek(delta);
}

void WebContentsImpl::OnMediaBrightnessChanged(float brightness) {
  MediaSession::Get(this)->OnBrightnessChanged(brightness);
}
...
void WebContentsImpl::HeavyExecutionDetected(int duration) {
  FOR_EACH_OBSERVER(WebContentsObserver, observers_, HeavyExecutionDetected(duration));
}
...
void WebContentsImpl::MediaAdjustBrightness(float delta) {
  delegate_->MediaAdjustBrightness(delta);
}

void WebContentsImpl::MediaSetRotateLock(bool lock) {
  delegate_->MediaSetRotateLock(lock);
}
