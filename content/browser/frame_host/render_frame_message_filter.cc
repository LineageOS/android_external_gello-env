...
bool RenderFrameMessageFilter::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameMessageFilter, message)
    ...
    IPC_MESSAGE_HANDLER(FrameHostMsg_OnWebActivity, OnWebActivity)
...
void RenderFrameMessageFilter::OnSetCookie(int render_frame_id,
  ...
  if (!WebRefiner::Get()->AllowSetCookie(render_process_id_, render_view_id,
...
void RenderFrameMessageFilter::OnGetCookies(int render_frame_id,
  ...
  context->cookie_store()->GetCookieListWithOptionsAsync(
      ...
      base::Bind(&RenderFrameMessageFilter::CheckPolicyForCookies, this,
                 render_frame_id, render_view_id, url, first_party_for_cookies, reply_msg));
}
...
void RenderFrameMessageFilter::OnWebActivity(
    int render_frame_id,
    int render_view_id,
    const GURL& url,
    const GURL& fp_url,
    int type) {
    WebRefiner::Get()->OnWebActivity(render_process_id_,
        render_view_id, render_frame_id, url, fp_url, type);
}
...
void RenderFrameMessageFilter::CheckPolicyForCookies(
  ...
  if (!WebRefiner::Get()->AllowGetCookies(render_process_id_, render_view_id, render_frame_id,
