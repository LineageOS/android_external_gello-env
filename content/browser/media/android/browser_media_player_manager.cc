...
void BrowserMediaPlayerManager::OnDeltaSeek(int player_id, int delta) {
  MediaPlayerAndroid* player = GetPlayer(player_id);
  DCHECK(player);

  base::TimeDelta seek_time = player->GetCurrentTime() + base::TimeDelta::FromSeconds(delta);

  if (seek_time < base::TimeDelta::FromSeconds(0)) {
    seek_time = base::TimeDelta::FromSeconds(0);
  }
