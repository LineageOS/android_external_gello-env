...
bool MediaWebContentsObserver::OnMessageReceived(
  ...
  IPC_BEGIN_MESSAGE_MAP_WITH_PARAM(MediaWebContentsObserver, msg,
    ...
    IPC_MESSAGE_HANDLER(MediaPlayerDelegateHostMsg_OnMediaPlaying,
                        OnMediaPlaying)
    IPC_MESSAGE_HANDLER(MediaPlayerDelegateHostMsg_OnMediaAdjustBrightness,
                        OnMediaAdjustBrightness)
    IPC_MESSAGE_HANDLER(MediaPlayerDelegateHostMsg_OnMediaSetRotateLock,
                        OnMediaSetRotateLock)
...
void MediaWebContentsObserver::OnMediaAdjustBrightness(
    RenderFrameHost* render_frame_host,
    float delta) {
  static_cast<WebContentsImpl*>(web_contents())->MediaAdjustBrightness(delta);
}

void MediaWebContentsObserver::OnMediaSetRotateLock(
    RenderFrameHost* render_frame_host,
    bool lock) {
  static_cast<WebContentsImpl*>(web_contents())->MediaSetRotateLock(lock);
}
