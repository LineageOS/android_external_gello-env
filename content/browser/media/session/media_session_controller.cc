...
void MediaSessionController::OnDeltaSeek(int player_id, int delta) {
  DCHECK_EQ(player_id_, player_id);
  media_web_contents_observer_->Send(
      new MediaPlayerDelegateMsg_DeltaSeek(
          id_.first->GetRoutingID(), id_.second, delta));
}

void MediaSessionController::OnBrightnessChanged(int player_id,
                                                 float brightness) {
  DCHECK_EQ(player_id_, player_id);
  media_web_contents_observer_->Send(
      new MediaPlayerDelegateMsg_BrightnessChanged(
          id_.first->GetRoutingID(), id_.second, brightness));
}
