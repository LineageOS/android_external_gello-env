...
void MediaSession::DeltaSeek(int delta) {
  for (const auto& it : players_)
    it.observer->OnDeltaSeek(it.player_id, delta);
}

void MediaSession::OnBrightnessChanged(float brightness) {
  for (const auto& it : players_)
    it.observer->OnBrightnessChanged(it.player_id, brightness);
}
