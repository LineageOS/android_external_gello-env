...
void RendererWebCookieJarImpl::setCookie(
  ...
  int render_view_id = sender_->render_view() ? sender_->render_view()->GetRoutingID() : MSG_ROUTING_NONE;
  sender_->Send(new FrameHostMsg_SetCookie(
      sender_->GetRoutingID(), render_view_id, url, first_party_for_cookies, value_utf8));
}
...
bool RendererWebCookieJarImpl::cookiesEnabled(
  ...
  sender_->Send(new FrameHostMsg_CookiesEnabled(
      sender_->GetRoutingID(), render_view_id, url, first_party_for_cookies, &cookies_enabled));
