...
blink::WebActivitiesClient* RenderFrameImpl::webActivitiesClient(blink::WebLocalFrame* frame) {
    return static_cast<WebActivitiesClient*>(this);
}
...
void RenderFrameImpl::didReceiveResponse(
  ...
  FOR_EACH_OBSERVER(
      RenderViewObserver, render_view_->observers_, DidReceiveResponse(frame_, response));
...
void RenderFrameImpl::onWebActivity(const blink::WebURL& url, const blink::WebURL& fp_url, int type) {
  int render_view_id = this->render_view() ? this->render_view()->GetRoutingID() : MSG_ROUTING_NONE;
  this->Send(new FrameHostMsg_OnWebActivity(this->GetRoutingID(), render_view_id, url, fp_url, type));
}
