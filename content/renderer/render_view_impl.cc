...
void RenderView::ApplyWebPreferences(const WebPreferences& prefs,
  ...
  settings->setNightModeEnabled(prefs.night_mode_enabled);
  settings->setPowersaveModeEnabled(prefs.powersave_mode_enabled);
...
RenderViewImpl* RenderViewImpl::Create(CompositorDependencies* compositor_deps,
  ...
  WebRefiner::Get()->RegisterRenderViewImpl(render_view);
  return render_view;
}
...
void RenderViewImpl::heavyExecutionDetected(int duration, bool page_load) {
  if (0 == frames_in_progress_ || page_load) {
    Send(new ViewHostMsg_HeavyExecutionDetected(routing_id_, duration));
  }
}
