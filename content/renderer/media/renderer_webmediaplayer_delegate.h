...
class CONTENT_EXPORT RendererWebMediaPlayerDelegate
 ...
 public:
  ...
  void AdjustBrightness(float delta) override;
  void SetRotateLock(bool lock) override;

  bool ShouldAllowBackgroundAudio() override;
 ...
 private:
  ...
  void OnMediaDelegateVolumeMultiplierUpdate(int delegate_id,
                                             double multiplier);
  void OnMediaDelegateDeltaSeek(int delegate_id, int delta);
  void OnMediaDelegateBrightnessChanged(int delegate_id, float brightness);
