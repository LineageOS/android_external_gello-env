...
bool RendererWebMediaPlayerDelegate::ShouldAllowBackgroundAudio() {
  const content::RendererPreferences& prefs =
      static_cast<content::RenderFrameImpl*>(
          render_frame())->render_view()->renderer_preferences();
  return prefs.enable_background_audio;
}

void RendererWebMediaPlayerDelegate::AdjustBrightness(float delta) {
  Send(new MediaPlayerDelegateHostMsg_OnMediaAdjustBrightness(routing_id(), delta));
}

void RendererWebMediaPlayerDelegate::SetRotateLock(bool lock) {
  Send(new MediaPlayerDelegateHostMsg_OnMediaSetRotateLock(routing_id(), lock));
}
...
bool RendererWebMediaPlayerDelegate::OnMessageReceived(
  ...
  IPC_BEGIN_MESSAGE_MAP(RendererWebMediaPlayerDelegate, msg)
    ...
    IPC_MESSAGE_HANDLER(MediaPlayerDelegateMsg_DeltaSeek,
                        OnMediaDelegateDeltaSeek)
    IPC_MESSAGE_HANDLER(MediaPlayerDelegateMsg_BrightnessChanged,
                        OnMediaDelegateBrightnessChanged)
...
void RendererWebMediaPlayerDelegate::OnMediaDelegateDeltaSeek(
    int delegate_id,
    int delta) {
  Observer* observer = id_map_.Lookup(delegate_id);
  if (observer)
    observer->OnDeltaSeek(delta);
}

void RendererWebMediaPlayerDelegate::OnMediaDelegateBrightnessChanged(
    int delegate_id,
    float brightness) {
  Observer* observer = id_map_.Lookup(delegate_id);
  if (observer)
