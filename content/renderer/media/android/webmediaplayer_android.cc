...
void WebMediaPlayerAndroid::OnShown() {
  ...
  if (hasVideo() && ShouldAllowVideoPausing()) {
    ResumeVideo();
  }
}

void WebMediaPlayerAndroid::OnSuspendRequested(bool must_suspend) {
  // If we're idle, pause and release resources; audio only
  ...
  if (must_suspend || (paused() && playback_completed_) || isFullscreen() ||
      !hasAudio() || !ShouldAllowBackgroundAudio()) {
    SuspendAndReleaseResources();
  } else if (hasVideo() && ShouldAllowVideoPausing()) {
...
void WebMediaPlayerAndroid::OnDeltaSeek(int delta) {
  player_manager_->DeltaSeek(player_id_, delta);
}
...
bool WebMediaPlayerAndroid::ShouldAllowBackgroundAudio() {
  const RendererPreferences& prefs = RenderFrameImpl::FromRoutingID(frame_id_)
                                         ->render_view()
                                         ->renderer_preferences();
  return prefs.enable_background_audio;
}

bool WebMediaPlayerAndroid::ShouldAllowVideoPausing() {
