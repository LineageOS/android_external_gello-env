...
public class MediaNotificationManager {
    ...
    private abstract static class ListenerService extends Service {
        ...
        private static final String ACTION_BACK10SEC =
                "NotificationMediaPlaybackControls.ListenerService.BACK10SEC";
        private static final String ACTION_FORWARD30SEC =
                "NotificationMediaPlaybackControls.ListenerService.FORWARD30SEC";
        ...
        private void processAction(Intent intent, MediaNotificationManager manager) {
            ...
            if (Intent.ACTION_MEDIA_BUTTON.equals(action)) {
                ...
                manager.mMediaNotificationInfo.listener.onDeltaSeek(BACKWARD_SEEK_PERIOD);
            } else if (ACTION_FORWARD30SEC.equals(action)) {
                manager.mMediaNotificationInfo.listener.onDeltaSeek(FORWARD_SEEK_PERIOD);
            }
        }
    }
    ...
    private void updateNotification() {
        ...
        } else if (mMediaNotificationInfo != null) {
          if (mLargeIconBridge == null) {
            mLargeIconBridge = new LargeIconBridge(Profile.getLastUsedProfile());
          }
          FavIconCallback callback = new FavIconCallback(mMediaNotificationInfo.origin);
          mLargeIconBridge.getLargeIconForUrl(
              mMediaNotificationInfo.origin, FAVICON_SIZE_DP, callback);
        }
    }

    private void updateNotificationWithFavIcon(Bitmap icon) {
        if (mService == null) return;
        ...
        // add bigview notification if JELLY_BEAN or newer
        if ((Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) && (icon != null)) {
          RemoteViews bigContentView = createBigContentView(icon);
    ...
    private class FavIconCallback implements LargeIconBridge.LargeIconCallback {
        ...
        public void onLargeIconAvailable(Bitmap icon, int fallbackColor) {
            if (icon == null) {
                RoundedIconGenerator roundedIconGenerator = new RoundedIconGenerator(
                        mContext, FAVICON_SIZE_DP, FAVICON_SIZE_DP,
                        FAVICON_CORNER_RADIUS_DP, fallbackColor,
                        FAVICON_TEXT_SIZE_DP);
                icon = roundedIconGenerator.generateIconForUrl(mUrl);
            }

            updateNotificationWithFavIcon(icon);
        }
    }

    private RemoteViews createBigContentView(Bitmap icon) {
        RemoteViews bigContentView =
                new RemoteViews(mContext.getPackageName(), R.layout.playback_notification_bar_large);
        bigContentView.setTextViewText(R.id.title, mMediaNotificationInfo.metadata.getTitle());
        bigContentView.setTextViewText(R.id.status, mMediaNotificationInfo.origin);
        bigContentView.setImageViewResource(R.id.icon, R.drawable.audio_playing);

        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            bigContentView.setViewVisibility(R.id.stop, View.VISIBLE);
            bigContentView.setOnClickPendingIntent(R.id.stop,
                    createPendingIntent(ListenerService.ACTION_STOP));
        }

        if (mMediaNotificationInfo.isPaused) {
            bigContentView.setImageViewResource(R.id.playpause, R.drawable.ic_vidcontrol_play);
            bigContentView.setContentDescription(R.id.playpause, mPlayDescription);
            bigContentView.setOnClickPendingIntent(R.id.playpause,
                    createPendingIntent(ListenerService.ACTION_PLAY));
        } else {
            bigContentView.setImageViewResource(R.id.playpause, R.drawable.ic_vidcontrol_pause);
            bigContentView.setContentDescription(R.id.playpause, mPauseDescription);
            bigContentView.setOnClickPendingIntent(R.id.playpause,
                    createPendingIntent(ListenerService.ACTION_PAUSE));
        }

        bigContentView.setOnClickPendingIntent(R.id.back10sec,
            createPendingIntent(ListenerService.ACTION_BACK10SEC));
        bigContentView.setOnClickPendingIntent(R.id.forward30sec,
            createPendingIntent(ListenerService.ACTION_FORWARD30SEC));

        bigContentView.setImageViewBitmap(R.id.favicon, icon);
