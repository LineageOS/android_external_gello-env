...
public class SingleWebsitePreferences extends PreferenceFragment
     ...
     */
    public static Website mergePermissionInfoForTopLevelOrigin(
            WebsiteAddress address, List<Set<Website>> websiteSets) {
        WebsiteAddress correctedAddress = null;
        String origin = address.getOrigin();
        if (TextUtils.isEmpty(origin)) { //Use the Host to fix the URL
            origin = UrlUtilities.fixupUrl(address.getTitle());
            // fixupurl always returns with a trailing slash. The preferences returned don't.
            if (origin.endsWith("/")) {
                origin = origin.substring(0, origin.length() - 1);
            }
            correctedAddress = WebsiteAddress.create(origin);
        }
        String host = Uri.parse(origin).getHost();
        Website merged = new Website(correctedAddress != null ? correctedAddress : address);
        ...
        for (Set<Website> websiteSet : websiteSets) {
            for (Website other : websiteSet) {
                ...
                if (merged.getJavaScriptException() == null &&
                    ...
                    if (other.getJavaScriptException().getPattern().equalsIgnoreCase(origin))
                        merged.setJavaScriptException(other.getJavaScriptException());
                }
                if (merged.getPopupException() == null &&
                        other.getPopupException() != null) {
                    if (other.getPopupException().getPattern().equalsIgnoreCase(origin))
                        merged.setPopupException(other.getPopupException());
                }
                if (merged.getCookieException() == null &&
                        other.getCookieException() != null) {
                    if (other.getCookieException().getPattern().equalsIgnoreCase(origin))
                        merged.setCookieException(other.getCookieException());
                }
    ...
    protected void setUpListPreference(Preference preference, ContentSetting value) {
        ...
        int index = (value == ContentSetting.ALLOW || value == ContentSetting.ASK ? 0 : 1);
