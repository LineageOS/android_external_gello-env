...
public class AppMenuPropertiesDelegate {
    ...
    public void prepareMenu(Menu menu) {
        ...
        if (isPageMenu && currentTab != null) {
            ...
            disableEnableMenuItem(menu, R.id.exit_menu_id,
                                  mActivity.getResources()
                                           .getBoolean(R.bool.swe_feature_exit_dialog),
                                  true,
                                  false);
        }

        if (isOverviewMenu) {
            if (isIncognito && !(CommandLine.getInstance().hasSwitch(ChromeSwitches.INCOGNITO_ONLY_MODE))) {
        ...
        incognitoItemVisible &= !(CommandLine.getInstance().hasSwitch(ChromeSwitches.INCOGNITO_ONLY_MODE));
