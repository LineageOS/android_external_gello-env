...
public class ChromeBrowserInitializer {
    ...
    /**
     * Initialize WebRefiner and WebDefender's global context and
     * apply their initial default and site specific preferences
     */
    static void initializeSecurityModules(Context ctx) {
        // WebRefiner
        WebRefinerPreferenceHandler.initializeGlobalInstance(ctx);
        WebRefinerPreferenceHandler.applyInitialPreferences();
        // WebDefender
        WebDefenderPreferenceHandler.initializeGlobalInstance(ctx);
        WebDefenderPreferenceHandler.applyInitialPreferences();
        // SecureConnect
        SecureConnectPreferenceHandler.initializeGlobalInstance(ctx);
        SecureConnectPreferenceHandler.applyInitialPreferences();
    }

    private void startChromeBrowserProcessesAsync(
            boolean startGpuProcess,
            final BrowserStartupController.StartupCallback callback) throws ProcessInitException {
        try {
            ...
            BrowserStartupController.get(mApplication, LibraryProcessType.PROCESS_BROWSER)
                    .startBrowserProcessesAsync(startGpuProcess, new BrowserStartupController.StartupCallback() {
                    @Override
                    public void onFailure() {
                        callback.onFailure();
                    }
                    ...
                    public void onSuccess(boolean alreadyStarted) {
                        ...
                        initializeSecurityModules(mApplication);
                    }
                });
        } finally {
