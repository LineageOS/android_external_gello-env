...
base::android::ScopedJavaLocalRef<jobject>
TemplateUrlServiceAndroid::GetPartnerTemplateUrlAt(JNIEnv* env,
                                                        jobject obj,
                                                        jint index) {
  TemplateURL* template_url = template_url_service_->GetTemplateURLs()[index];
  if (IsPrepopulatedTemplate(template_url) &&
...
static base::ListValue& overrides = *new base::ListValue;
// Starting default id for search engines.
int se_default_id = 1001;

void TemplateUrlServiceAndroid::ClearPrepopulatedEnginesInPrefs() {
  PrefService* prefs = GetOriginalProfile()->GetPrefs();
  ...
  update->Swap(&overrides);
  template_url_service_->RepairPrepopulatedSearchEngines();
  overrides.Clear();
}

void TemplateUrlServiceAndroid::AddSearchEngine(JNIEnv* env,
    jobject obj, jstring jtitle, jstring jkeyword,
    jstring jsearch_url, jstring jfavicon_url) {
  std::unique_ptr<base::DictionaryValue> entry(new base::DictionaryValue);
  // Set only the minimal required settings for a search provider configuration.
  entry->SetString("name", base::android::ConvertJavaStringToUTF16(env, jtitle));
  entry->SetString("keyword", base::android::ConvertJavaStringToUTF16(env, jkeyword));
  entry->SetString("search_url", base::android::ConvertJavaStringToUTF8(env, jsearch_url));
  entry->SetString("favicon_url", base::android::ConvertJavaStringToUTF16(env, jfavicon_url));
  entry->SetString("encoding", "UTF-8");
  entry->SetInteger("id", se_default_id++);
  overrides.Append(entry->DeepCopy());
}

void TemplateUrlServiceAndroid::MakeDefault(JNIEnv* env,
    jobject obj, jstring jkeyword) {
  ClearPrepopulatedEnginesInPrefs();
  TemplateURL* url = template_url_service_->GetTemplateURLForKeyword(
