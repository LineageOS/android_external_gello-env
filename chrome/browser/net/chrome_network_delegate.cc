...
ChromeNetworkDelegate::ChromeNetworkDelegate(
  ...
  // Eg: <Header1>: <Value1>\r\n<Header2>: <Value2>\r\n
  additional_http_headers_ = command_line->GetSwitchValueASCII(switches::kHTTPHeaders);
#endif
}
...
int ChromeNetworkDelegate::OnBeforeURLRequest(
  ...
  {
      int result = net::OK;
      content::WebRefiner::Get()->OnBeforeURLRequest(request, new_url, &result);
      if (result != net::OK) {
        request->net_log().AddEvent(
            net::NetLog::TYPE_CHROME_POLICY_ABORTED_REQUEST,
...
int ChromeNetworkDelegate::OnBeforeSendHeaders(
  ...
  content::WebRefiner::Get()->OnBeforeSendHeaders(request, headers);
#if defined(OS_ANDROID)
  headers->AddHeadersFromString(additional_http_headers_);
#endif
...
int ChromeNetworkDelegate::OnHeadersReceived(
  ...
  content::WebRefiner::Get()->OnHeadersReceived(request,
...
void ChromeNetworkDelegate::OnCompleted(net::URLRequest* request,
  ...
  content::WebRefiner::Get()->OnCompleted(request, started);
