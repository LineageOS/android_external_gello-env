...
void SafeBrowsingService::OnResourceRequest(const net::URLRequest* request, content::ResourceType resource_type) {
#if defined(FULL_SAFE_BROWSING)
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  ...
  // If the browser is being used to load new pages, reset the update timer
  // to keep the safe browsing updates running.
  // Only main frame page loads are used to avoid background page requests.
  if (resource_type == content::RESOURCE_TYPE_MAIN_FRAME) {
    if (protocol_manager_)
      protocol_manager_->ResetUpdateTimer();
  }
#endif
}
