...
// Update this check based on user preference (Wifi only or Both) from UI.
static bool kSbEnableCellularUpdates = true;

// Update backoff counter if browser is not used.
static int kSbBackOffCounter = 0;
...
SafeBrowsingProtocolManager::SafeBrowsingProtocolManager(
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch("disable-safebrowsing-cellular-update"))
    kSbEnableCellularUpdates = false;
}

bool GetCellularUpdatesEnabled() {
  ...
  // the legacy value(to preserve prior behavior).
  if (!g_browser_process->local_state()) {
    return kSbEnableCellularUpdates;
  }

  return kSbEnableCellularUpdates &&
  ProfileManager::GetActiveUserProfile()->GetOriginalProfile()->GetPrefs()->
  GetBoolean(prefs::kSecurityUpdatesCellularAllowed);
}
...
void SafeBrowsingProtocolManager::GetNextUpdate() {
  ...
  // Update on Wifi only.
  if (!GetCellularUpdatesEnabled() && net::NetworkChangeNotifier::GetConnectionType() ==
      net::NetworkChangeNotifier::CONNECTION_WIFI) {
    IssueUpdateRequest();
  }
  // Update on both: Wifi and Cellular.
  else if (GetCellularUpdatesEnabled()) {
    IssueUpdateRequest();
  }
}
...
// Reset the update timer if the browser is being used
// If the browser is not used, the update interval will continue to double.
void SafeBrowsingProtocolManager::ResetUpdateTimer() {
  // If the timer is not running (If cellular updates were turned off,
  // the timer will stop), enable the timer again when the browser is used.
  if (!update_timer_.IsRunning() ||
      update_timer_.GetCurrentDelay() > next_update_interval_) {
    kSbBackOffCounter = 0;
...
base::TimeDelta SafeBrowsingProtocolManager::GetNextUpdateInterval(
  ...
  // Double the update time if the browser is not used
  if (kSbBackOffCounter > 0)
    next = next_update_interval_ * std::pow(2, kSbBackOffCounter);
  // Limit the max update interval to 5 days (30m*2^8=128h)
  if (kSbBackOffCounter < 9)
    kSbBackOffCounter++;
