...
TEST_F(SafeBrowsingProtocolManagerTest, TestBackOffTimes) {
  ...
  // The test result is replaced by the new SWE algorithm to keep doubling
  // the update timer to reduce BW.
  next = pm->GetNextUpdateInterval(false);
  EXPECT_EQ(next, TimeDelta::FromSeconds(1800 * std::pow(2,8)));
}
