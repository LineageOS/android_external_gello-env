...
void TabWebContentsDelegateAndroid::MediaAdjustBrightness(float delta) {
  JNIEnv* env = base::android::AttachCurrentThread();
  ScopedJavaLocalRef<jobject> obj = GetJavaDelegate(env);
  if (obj.is_null())
    return;
  Java_TabWebContentsDelegateAndroid_mediaAdjustBrightness(
      env, obj.obj(), delta);
}

void TabWebContentsDelegateAndroid::MediaSetRotateLock(bool lock) {
  JNIEnv* env = base::android::AttachCurrentThread();
  ScopedJavaLocalRef<jobject> obj = GetJavaDelegate(env);
  if (obj.is_null())
    return;
  Java_TabWebContentsDelegateAndroid_mediaSetRotateLock(env, obj.obj(), lock);
}
...
void OnMediaBrightnessChanged(JNIEnv* env,
                          const JavaParamRef<jclass>& clazz,
                          const JavaParamRef<jobject>& java_web_contents,
                          float brightness) {
  content::WebContents* web_contents =
          content::WebContents::FromJavaWebContents(java_web_contents);
  web_contents->OnMediaBrightnessChanged(brightness);
}
