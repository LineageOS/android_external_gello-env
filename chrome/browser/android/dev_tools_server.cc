...
class UnixDomainServerSocketFactory
 ...
 private:
  std::unique_ptr<net::ServerSocket> CreateForHttpServer() override {
    ...
    //SWE Browser prefixes socket_name_ with "webview" to ensure it works with     151
    //Chromedriver. In case of SWE always try fallback option which is expected 152
    //format for Chromedriver.
    if(strncmp(socket_name_.c_str(), "webview", 7) != 0) {
