...
void ThumbnailCache::RemoveFromDiskWithIdMask(TabId id, int mask) {
  base::Closure remove_task =
      base::Bind(&ThumbnailCache::RemoveFromDiskWithIdMaskTask,
                 id, mask);
  content::BrowserThread::PostTask(
      content::BrowserThread::FILE, FROM_HERE, remove_task);
}
...
void ThumbnailCache::RemoveFromDiskWithIdMaskTask(TabId id, int mask) {
  base::FilePath dir_path = GetCacheDirectory();
  base::FileEnumerator enumerator(dir_path, false, base::FileEnumerator::FILES);
  while (true) {
    base::FilePath path = enumerator.Next();
    ...
    TabId tab_id;
    bool success = base::StringToInt(info.GetName().value(), &tab_id);
    if (success && ((tab_id & mask) == (id & mask)))
      base::DeleteFile(path, false);
  }
}
...
void ThumbnailCache::CompressionTask(
  ...
  if (!raw_data.empty()) {
    ...
    auto compressor = cc::TextureCompressor::Create(
        cc::TextureCompressor::kFormatETC1,
        cc::TextureCompressor::kColorFormatRGBA);
    compressor->Compress(
        reinterpret_cast<uint8_t *>(raw_data.getPixels()),
        reinterpret_cast<uint8_t *>(etc1_pixel_ref->pixels()),
        raw_data_size.width(),
        raw_data_size.height(),
        cc::TextureCompressor::kQualityMedium);
