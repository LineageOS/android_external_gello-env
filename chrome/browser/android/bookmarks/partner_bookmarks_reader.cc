...
void SetFaviconTask(Profile* profile,
  ...
  gfx::Size pixel_size(width, height);
...
void SetFaviconCallback(Profile* profile,
  ...
  SetFaviconTask(profile, page_url, icon_url, image_data, icon_type, width, height);
...
void PrepareAndSetFavicon(JNIEnv* env, jbyte* icon_bytes, int icon_len,
  ...
  int width = icon_bitmap.width();
  int height = icon_bitmap.height();
  ...
  if (BrowserThread::CurrentlyOn(BrowserThread::UI)) {
    SetFaviconTask(profile,
                   ...
                   image_data, icon_type, width, height);
  } else {
    ...
    BrowserThread::PostTask(
        ...
        base::Bind(&SetFaviconCallback,
                   ...
                   image_data, icon_type, width, height, &event));
