...
static jboolean IsContentSettingManaged(JNIEnv* env,
  ...
  static jboolean IsRestrictedToSecureOrigins(JNIEnv* env,
                                            const JavaParamRef<jobject>& obj,
                                            int content_settings_type) {
  ...
  if (PermissionUtil::GetPermissionType(type, &permission_type)) {
    PermissionManager* manager = PermissionManager::Get(GetOriginalProfile());
    if (manager == NULL) return false;
    PermissionContextBase* context = manager->GetPermissionContext(permission_type);
    if (context == NULL) return false;
    bool result = context->IsRestrictedToSecureOrigins();
    return result;
  }
  return false;
}

static ScopedJavaLocalRef<jstring> GetExpiryForContentType(JNIEnv* env,
                                        const JavaParamRef<jclass>& clazz,
                                        int content_settings_type,
                                        const JavaParamRef<jstring>& pattern) {
  DCHECK(content_settings_type == CONTENT_SETTINGS_TYPE_GEOLOCATION ||
         content_settings_type == CONTENT_SETTINGS_TYPE_MEDIASTREAM_CAMERA ||
         content_settings_type == CONTENT_SETTINGS_TYPE_MEDIASTREAM_MIC);

  GURL origin_url = GURL(ConvertJavaStringToUTF8(env, pattern));
  HostContentSettingsMap* host_content_settings_map =
      HostContentSettingsMapFactory::GetForProfile(GetOriginalProfile());
  base::Time expire_in = host_content_settings_map->
      GetExpiry(origin_url, origin_url,
        static_cast<ContentSettingsType>(content_settings_type));

  return ConvertUTF8ToJavaString(env, base::Int64ToString(expire_in.ToJavaTime()));
}
...
static void SetSecureConnectEnabled(JNIEnv* env,
                                 const JavaParamRef<jobject>& obj,
                                 jboolean is_enabled) {
  HostContentSettingsMap* host_content_settings_map =
      HostContentSettingsMapFactory::GetForProfile(GetOriginalProfile());
  host_content_settings_map->SetDefaultContentSetting(
      CONTENT_SETTINGS_TYPE_SECURE_CONNECT,
      is_enabled ? CONTENT_SETTING_ALLOW : CONTENT_SETTING_BLOCK);
}

static jboolean GetSecureConnectEnabled(JNIEnv* env,
                                        const JavaParamRef<jobject>& obj) {
  return GetBooleanForContentSetting(CONTENT_SETTINGS_TYPE_SECURE_CONNECT);
}


static void SetWebRefinerEnabled(JNIEnv* env,
                                 const JavaParamRef<jobject>& obj,
                                 jboolean is_enabled) {
  HostContentSettingsMap* host_content_settings_map =
      HostContentSettingsMapFactory::GetForProfile(GetOriginalProfile());
  host_content_settings_map->SetDefaultContentSetting(
      CONTENT_SETTINGS_TYPE_WEBREFINER,
      is_enabled ? CONTENT_SETTING_ALLOW : CONTENT_SETTING_BLOCK);
}

static jboolean GetWebRefinerEnabled(JNIEnv* env,
                                          const JavaParamRef<jobject>& obj) {
  return GetBooleanForContentSetting(CONTENT_SETTINGS_TYPE_WEBREFINER);
}

static void SetWebDefenderEnabled(JNIEnv* env,
                                  const JavaParamRef<jobject>& obj,
                                  jboolean is_enabled) {
  HostContentSettingsMap* host_content_settings_map =
      HostContentSettingsMapFactory::GetForProfile(GetOriginalProfile());
  host_content_settings_map->SetDefaultContentSetting(
      CONTENT_SETTINGS_TYPE_WEBDEFENDER,
      is_enabled ? CONTENT_SETTING_ALLOW : CONTENT_SETTING_BLOCK);
}

static jboolean GetWebDefenderEnabled(JNIEnv* env,
                                          const JavaParamRef<jobject>& obj) {
  return GetBooleanForContentSetting(CONTENT_SETTINGS_TYPE_WEBDEFENDER);
}
...
static ScopedJavaLocalRef<jobject> GetAboutVersionStrings(
  ...
  application.append(" ");
  application.append(version_info::GetSWEVersionNumber());
...
static void SetSecurityUpdatesOnCellular(JNIEnv* env,
                                const JavaParamRef<jobject>& obj,
                                jboolean enabled) {
  GetPrefService()->SetBoolean(prefs::kSecurityUpdatesCellularAllowed,
                               enabled);
}

static jboolean GetSecurityUpdatesOnCellular(JNIEnv* env,
  const JavaParamRef<jobject>& obj) {
  return GetPrefService()->GetBoolean(prefs::kSecurityUpdatesCellularAllowed);
}


static void SetNightModeEnabled(JNIEnv* env,
                                const JavaParamRef<jobject>& obj,
                                jboolean nightModeEnabled) {
  GetPrefService()->SetBoolean(prefs::kWebKitNightModeEnabled,
                               nightModeEnabled);
}

static jboolean GetNightModeEnabled(JNIEnv* env,
                                    const JavaParamRef<jobject>& obj) {
  return GetPrefService()->GetBoolean(prefs::kWebKitNightModeEnabled);
}

static void SetPowersaveModeEnabled(JNIEnv* env,
                                const JavaParamRef<jobject>& obj,
                                jboolean powersaveModeEnabled) {
  GetPrefService()->SetBoolean(prefs::kWebKitPowersaveModeEnabled,
                               powersaveModeEnabled);
}

static jboolean GetPowersaveModeEnabled(JNIEnv* env, const JavaParamRef<jobject>& obj) {
...
static jboolean GetBackgroundAudioEnabled(JNIEnv* env,
                                          const JavaParamRef<jobject>& obj) {
