...
static void GetWebRefinerOrigins(JNIEnv* env,
                                 const JavaParamRef<jclass>& clazz,
                                 const JavaParamRef<jobject>& list,
                                 jboolean managedOnly) {
  GetOrigins(env, CONTENT_SETTINGS_TYPE_WEBREFINER,
    &Java_WebsitePreferenceBridge_insertWebRefinerInfoIntoList,
    list, managedOnly);
}

static jint GetWebRefinerSettingForOrigin(JNIEnv* env,
                                          const JavaParamRef<jclass>& clazz,
                                          const JavaParamRef<jstring>& origin,
                                          const JavaParamRef<jstring>& embedder,
                                          jboolean is_incognito) {
  return GetSettingForOrigin(env, CONTENT_SETTINGS_TYPE_WEBREFINER, origin,
                             embedder, is_incognito);
}

static void SetWebRefinerSettingForOrigin(JNIEnv* env,
                                          const JavaParamRef<jclass>& clazz,
                                          const JavaParamRef<jstring>& origin,
                                          const JavaParamRef<jstring>& embedder,
                                          jint value,
                                          jboolean is_incognito) {
  SetSettingForOrigin(env, CONTENT_SETTINGS_TYPE_WEBREFINER, origin, nullptr,
                      static_cast<ContentSetting>(value), is_incognito);
}

static void GetWebDefenderOrigins(JNIEnv* env,
                                 const JavaParamRef<jclass>& clazz,
                                 const JavaParamRef<jobject>& list,
                                 jboolean managedOnly) {
  GetOrigins(env, CONTENT_SETTINGS_TYPE_WEBDEFENDER,
  Java_WebsitePreferenceBridge_insertWebDefenderInfoIntoList,
  list, managedOnly);
}

static jint GetWebDefenderSettingForOrigin(JNIEnv* env,
                                          const JavaParamRef<jclass>& clazz,
                                          const JavaParamRef<jstring>& origin,
                                          const JavaParamRef<jstring>& embedder,
                                          jboolean is_incognito) {
  return GetSettingForOrigin(env, CONTENT_SETTINGS_TYPE_WEBDEFENDER, origin,
                             embedder, is_incognito);
}

static void SetWebDefenderSettingForOrigin(JNIEnv* env,
                                          const JavaParamRef<jclass>& clazz,
                                          const JavaParamRef<jstring>& origin,
                                          const JavaParamRef<jstring>& embedder,
                                          jint value,
                                          jboolean is_incognito) {
  SetSettingForOrigin(env, CONTENT_SETTINGS_TYPE_WEBDEFENDER, origin, nullptr,
                      static_cast<ContentSetting>(value), is_incognito);
}
