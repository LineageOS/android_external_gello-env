...
#if DISABLED_BY_UPLEVEL
  // Change-Id: Icf8c6f969bee347db366298382474410058b630f is no longer appropriate

  // Skip AddChild if the layer_ is already ancestor of content_layer's cc
  // layer. Adding a child causes the stacking order property to change, which
  // causes cc damage tracker to include the entire layer rect as part of the
  // damage rect.
  if ( content_layer_->layer() &&
       (content_layer_->layer()->parent() != layer_.get())) {
    layer_->AddChild(content_layer_->layer());
  }
#endif
