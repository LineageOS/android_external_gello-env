...
void TabContentManager::CacheTabWithID(JNIEnv* env,
                                 jobject obj,
                                 jobject tab,
                                 jint id,
...
void TabContentManager::CacheBitmapWithID(JNIEnv* env,
                                          jobject obj,
                                          jint id,
                                          jobject bitmap,
                                          jfloat thumbnail_scale) {
  if (thumbnail_cache_->CheckAndUpdateThumbnailMetaData(id, GURL(""))) {
    gfx::JavaBitmap java_bitmap_lock(bitmap);
    SkBitmap skbitmap = gfx::CreateSkBitmapFromJavaBitmap(java_bitmap_lock);
    skbitmap.setImmutable();
    PutThumbnailIntoCache(id, thumbnail_scale, skbitmap);
  }
}
...
void TabContentManager::RemoveTabThumbnailWithID(JNIEnv* env,
                                                 jobject obj,
                                                 jint id, jint mask) {
    thumbnail_cache_->RemoveFromDiskWithIdMask(id, mask);
}
