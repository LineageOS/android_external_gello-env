...
void GeolocationPermissionContext::DecidePermission(
  ...
  const GURL& primary_url = requesting_origin.GetOrigin();
  ...
  switch (content_setting) {
    case CONTENT_SETTING_ALLOW_24H:
      expire =  base::Time::Now() - HostContentSettingsMapFactory::GetForProfile(profile_)->GetExpiry(
          requesting_origin.GetOrigin(),
          secondary_url,
          CONTENT_SETTINGS_TYPE_GEOLOCATION);

      if(expire.InSecondsF() > 0) {
        HostContentSettingsMapFactory::GetForProfile(profile_)->SetContentSettingCustomScope(
