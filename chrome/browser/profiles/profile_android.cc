...
bool ProfileAndroid::Encrypt( const base::string16& plain_text,
    std::string* cipher_text) {
  bool result = false;
  JNIEnv* env = AttachCurrentThread();
  if (obj_.is_null() || cipher_text == NULL)
    return result;
  ...
  ScopedJavaLocalRef<jbyteArray> plain_text_ref =
      base::android::ToJavaByteArray(env,
                      reinterpret_cast<const uint8_t*>(text_str.data()),
                      text_str.length());
  ...
  ScopedJavaLocalRef<jbyteArray> encrypt_ref =
      Java_Profile_encrypt(
          env,
          obj_.obj(),
          plain_text_ref.obj());

  std::vector<unsigned char> encrypted;
  base::android::JavaByteArrayToByteVector(env, encrypt_ref.obj(), &encrypted);
...
bool ProfileAndroid::Decrypt(const std::string& cipher_text,
    base::string16* plain_text) {
  JNIEnv* env = AttachCurrentThread();
  bool result = false;
  if (obj_.is_null() || plain_text == NULL)
    return result;

  ScopedJavaLocalRef<jbyteArray> cipher_text_ref =
    base::android::ToJavaByteArray(env,
                    reinterpret_cast<const uint8_t*>(cipher_text.data()),
                    cipher_text.length());
  DCHECK(!cipher_text_ref.is_null());

  // Invoke API
  ScopedJavaLocalRef<jbyteArray> decrypt_ref =
      Java_Profile_decrypt(
          env,
          obj_.obj(),
          cipher_text_ref.obj());

  std::vector<unsigned char> decrypted;
  base::android::JavaByteArrayToByteVector(env, decrypt_ref.obj(), &decrypted);
