...
bool ChromePasswordManagerClient::EncryptMatch(autofill::PasswordForm* preferred_match) {
    ProfileAndroid* profile = ProfileAndroid::FromProfile(profile_);
    // Encrypt the password !!
    std::string encrypted_str;
    bool result = profile->Encrypt(preferred_match->password_value, &encrypted_str);
...
bool ChromePasswordManagerClient::DecryptMatch(autofill::PasswordForm* preferred_match) {
  ...
  if (!base64_encrypted_str.empty()) {
    ...
    result = profile->Decrypt(encrypted_str, &decrypted_str);
    if (result) {
