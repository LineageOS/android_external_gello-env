...
ContentSetting MediaPermission::GetStoredContentSetting(
  ...
  if (is_insecure_pepper_request_) {
    ...
    ContentSetting setting = media_device_permission_context->GetPermissionStatus(
        requesting_origin_, embedding_origin_);
    if (setting == CONTENT_SETTING_ALLOW_24H) {
      base::TimeDelta expire =
        base::Time::Now() - HostContentSettingsMapFactory::GetForProfile(profile_)
        ->GetExpiry(requesting_origin_, embedding_origin_, content_type_);
