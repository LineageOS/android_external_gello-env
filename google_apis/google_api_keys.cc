 ...
 public:
  APIKeyCache() {
    ...
    api_key_ = CalculateKeyValue(
        GOOGLE_API_KEY, STRINGIZE_NO_EXPANSION(GOOGLE_API_KEY), switches::kGoogleApisKey,
...
// A special non-stable key is at the moment defined only for Android Chrome.
    ...
    api_key_non_stable_ = CalculateKeyValue(
            ...
            STRINGIZE_NO_EXPANSION(GOOGLE_API_KEY_PHYSICAL_WEB_TEST), switches::kGoogleApisKey,
            std::string(), environment.get(), command_line);
#else
